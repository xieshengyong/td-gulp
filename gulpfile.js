...
var txpath2 = '//lol.qq.com/m/act/a20170717lolmusic/';
var txcss = 'handover/';
var txjs = 'handover/';
var tximg = 'handover/ossweb-img/';
...
gulp.task('lessmin', function () {
    return gulp.src(lessSrc)
        ...
        .pipe(cssmin())
        .pipe(gulp.dest(txcss));
});

gulp.task('rhtml', function () {
    return gulp.src('index.html')
        .pipe(replace(/"dist\/css\//g, '"' + txpath2))
        .pipe(replace(/"dist\/js\//g, '"' + txpath2))
        .pipe(gulp.dest('handover/'));
});

gulp.task('rjs', ['webpack:build'], function () {
    return gulp.src(txjs + 'main.js')
...
gulp.task('webpack:build', function (callback) {
    ...
    myConfig.output.filename = txjs + 'main.js';

    myConfig.module.loaders[0] = {test: /js[\/|\\]lib[\/||\\][\w|\.|_|-]+js$/, loader: 'url-loader?importLoaders=1&limit=1000&name='+txjs+'[name].[ext]'};
